...
//        assertNotNull("Intersection is null for " + typesToIntersect, result);
    ...
    private void assertType(String expression, JetType expectedType) {
        ...
        JetType type = expressionTypingServices.getType(ScopesPackage.asJetLocalScope(scopeWithImports), jetExpression, TypeUtils.NO_EXPECTED_TYPE, DataFlowInfo.EMPTY, JetTestUtils.DUMMY_TRACE);
    ...
    private void assertErrorType(String expression) {
        Project project = getProject();
        JetExpression jetExpression = JetPsiFactory(project).createExpression(expression);
        JetType type = expressionTypingServices.safeGetType(ScopesPackage.asJetLocalScope(scopeWithImports), jetExpression, TypeUtils.NO_EXPECTED_TYPE, DataFlowInfo.EMPTY, JetTestUtils.DUMMY_TRACE);
    ...
    private void assertType(JetScope scope, String expression, String expectedTypeStr) {
        ...
        JetType type = expressionTypingServices.getType(
                ScopesPackage.asJetLocalScope(addImports(scope)), jetExpression, TypeUtils.NO_EXPECTED_TYPE, DataFlowInfo.EMPTY, new BindingTraceContext());
    ...
    private JetType makeType(JetScope scope, String typeStr) {
        return typeResolver.resolveType(ScopesPackage.asJetLocalScope(scope), JetPsiFactory(getProject()).createType(typeStr), JetTestUtils.DUMMY_TRACE, true);
    }
}
