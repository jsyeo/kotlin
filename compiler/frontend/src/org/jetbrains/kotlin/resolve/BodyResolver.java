...
public class BodyResolver {
    ...
    private void resolveSecondaryConstructors(@NotNull BodiesResolveContext c) {
        for (Map.Entry<JetSecondaryConstructor, ConstructorDescriptor> entry : c.getSecondaryConstructors().entrySet()) {
            JetLocalScope declaringScope = c.getDeclaringScope(entry.getKey());
    ...
    public void resolveSecondaryConstructorBody(
            ...
            @NotNull JetLocalScope declaringScope
    ) {
        ...
        resolveFunctionBody(outerDataFlowInfo, trace, constructor, descriptor, declaringScope,
                            new Function1<JetLocalScope, DataFlowInfo>() {
                                @Override
                                public DataFlowInfo invoke(@NotNull JetLocalScope headerInnerScope) {
    ...
    public void resolveDelegationSpecifierList(
            ...
            @NotNull JetLocalScope scopeForSupertypeResolution,
            @NotNull final JetLocalScope scopeForMemberResolution
    ) {
        final JetLocalScope scopeForConstructor = primaryConstructor == null
                ? null
        ...
        JetVisitorVoid visitor = new JetVisitorVoid() {
            ...
            public void visitDelegationByExpressionSpecifier(@NotNull JetDelegatorByExpressionSpecifier specifier) {
                ...
                if (delegateExpression != null) {
                    JetLocalScope scope = scopeForConstructor == null ? scopeForMemberResolution : scopeForConstructor;
    ...
    public void resolveAnonymousInitializer(
        ...
        JetLocalScope scopeForInitializers = classDescriptor.getScopeForInitializerResolution();
    ...
    private void processModifiersOnInitializer(@NotNull JetModifierListOwner owner, @NotNull JetLocalScope scope) {
    ...
    private void resolvePrimaryConstructorParameters(@NotNull BodiesResolveContext c) {
        for (Map.Entry<JetClassOrObject, ClassDescriptorWithResolutionScopes> entry : c.getDeclaredClasses().entrySet()) {
            ...
            if (unsubstitutedPrimaryConstructor != null) {
                ...
                JetLocalScope parameterScope = getPrimaryConstructorParametersScope(classDescriptor.getScopeForClassHeaderResolution(),
    ...
    private static JetLocalScope getPrimaryConstructorParametersScope(
            JetLocalScope originalScope,
    ...
    private void resolveProperty(
            ...
            @Nullable JetLocalScope parentScope,
        ...
        JetLocalScope propertyScope = getScopeForProperty(c, property);
        if (parentScope == null) {
    ...
    private JetLocalScope makeScopeForPropertyAccessor(@NotNull BodiesResolveContext c, @NotNull JetPropertyAccessor accessor, @NotNull PropertyDescriptor descriptor) {
        JetLocalScope accessorDeclaringScope = c.getDeclaringScope(accessor);
    ...
    public void resolvePropertyAccessors(
        ...
        if (getter != null && getterDescriptor != null) {
            JetLocalScope accessorScope = makeScopeForPropertyAccessor(c, getter, propertyDescriptor);
        ...
        if (setter != null && setterDescriptor != null) {
            JetLocalScope accessorScope = makeScopeForPropertyAccessor(c, setter, propertyDescriptor);
    ...
    public void resolvePropertyDelegate(
            ...
            @NotNull JetLocalScope parentScopeForAccessor,
            @NotNull JetLocalScope propertyScope
    ) {
        ...
        JetLocalScope propertyDeclarationInnerScope = JetScopeUtils.getPropertyDeclarationInnerScopeForInitializer(
                propertyDescriptor, propertyScope, propertyDescriptor.getTypeParameters(), NO_RECEIVER_PARAMETER, trace);
        JetLocalScope accessorScope = JetScopeUtils.makeScopeForPropertyAccessor(
    ...
    @NotNull
    private static JetLocalScope getScopeForProperty(@NotNull BodiesResolveContext c, @NotNull JetProperty property) {
        JetLocalScope scope = c.getDeclaringScope(property);
    ...
    private void resolveFunctionBodies(@NotNull BodiesResolveContext c) {
        for (Map.Entry<JetNamedFunction, SimpleFunctionDescriptor> entry : c.getFunctions().entrySet()) {
            ...
            JetLocalScope scope = c.getDeclaringScope(declaration);
    ...
    public void resolveFunctionBody(
            ...
            @NotNull FunctionDescriptor functionDescriptor,
            @NotNull JetLocalScope scope,
            @Nullable Function1<JetLocalScope, DataFlowInfo> beforeBlockBody,
            @NotNull CallChecker callChecker
    ) {
        JetLocalScope innerScope = FunctionDescriptorUtil.getFunctionInnerScope(scope, functionDescriptor, trace);
    ...
    public void resolveConstructorParameterDefaultValuesAndAnnotations(
        ...
        JetLocalScope scope = getPrimaryConstructorParametersScope(declaringScope, constructorDescriptor);
