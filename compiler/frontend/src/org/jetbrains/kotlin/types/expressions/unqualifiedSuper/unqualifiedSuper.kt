...
private val METHOD_OF_ANY_ARITY = hashMapOf("hashCode" to 0, "equals" to 1, "toString" to 0)

private fun isCallingMethodOfAny(callExpression: JetCallExpression, calleeName: Name): Boolean =
        METHOD_OF_ANY_ARITY.getOrElse(calleeName.asString(), { -1 }) == callExpression.valueArguments.size()

public fun isPossiblyAmbiguousUnqualifiedSuper(superExpression: JetSuperExpression, supertypes: Collection<JetType>): Boolean =
        supertypes.size() > 1 ||
        (supertypes.size() == 1 && !supertypes.single().canBeSuperclass() && isCallingMethodOfAnyWithSuper(superExpression))
...
private fun JetType.canBeSuperclass(): Boolean =
        when (TypeUtils.getClassDescriptor(this)?.kind) {
            ClassKind.CLASS,
...
private fun isConcreteMember(supertype: JetType, memberDescriptor: MemberDescriptor): Boolean {
    // Explicitly inherited from Any?
    val classDescriptorForSupertype = TypeUtils.getClassDescriptor(supertype)
    if (classDescriptorForSupertype != null && KotlinBuiltIns.isAny(classDescriptorForSupertype))
        return true
